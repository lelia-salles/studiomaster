    ...
    for (auto & pinfo : mPendingUsers) {
        ...
        if (pinfo.second.failed) {
            if (pinfo.second.blocked) {
                ...
                ppvf->messageLabel->setText(TRANS("Could not connect with user, one or both of you may need to configure your internal firewall or network router to allow StudioMaster to work between you. See the help documentation to enable port forwarding on your router."), dontSendNotification);
                ppvf->removeButton->setVisible(true);
            }
        }
        else {
